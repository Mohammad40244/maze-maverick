...
int incrementNumberInFile(string fileName, bool incrementSecondLine , int newNumber) ;
void savePlayerData(const vector<Player>& players);
vector<Player> loadPlayerData() ;
void inputPlayerData( vector<Player>& players);
...
void main_output(){
    ...
    if(x == '1'){
        ...
        if (x == '1'){
            ...
            cin >> length ;
            if ((length> soton*linee) or (length < soton + linee -2) or (length%2 != (soton+linee)%2)){
...
//                copycopymaze.shrink_to_fit();
        ...
        cout << name << " won " ;
        getline(names >> ws , receiver);
        cout << receiver << " times " << endl << name << " has played " ;
        getline(names >> ws , receiver);
        cout << receiver << " times " << endl << "total time : " ;
        getline(names >> ws , receiver);
        cout << receiver << endl  ;
        getline(names >> ws , receiver);
        cout << receiver << endl ;
...
void playground (vector<vector<int>> custom_maze ){
    ...
    if (num == custom_maze[x][y]){
        ...
        incrementNumberInFile (username , true , timesecond-timebase);
        vector<Player> players = loadPlayerData();
        savePlayerData(players);
        vector<Player> loadPlayerData() ;
        inputPlayerData(players);
        ...
        createFile (username);
        username = username+".txt" ;
        incrementNumberInFile (username , false , timesecond-timebase);
        vector<Player> players = loadPlayerData();
        savePlayerData(players);
        vector<Player> loadPlayerData() ;
        inputPlayerData(players);
